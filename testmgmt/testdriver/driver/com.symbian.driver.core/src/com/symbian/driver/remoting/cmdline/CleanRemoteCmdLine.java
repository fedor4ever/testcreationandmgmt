...
/**
 * Class representing the command line 'cleanjob'.
 * 
 * @author EngineeringTools
 */
public class CleanRemoteCmdLine extends CmdLine {
	
	/**
	 * Standard constructor. Sets the switches -j and --srv
	 */
	public CleanRemoteCmdLine() {
		super("cleanremote", new CmdOperational() {
			...
			public synchronized Object run(final CmdLine aCmd) throws ParseException {
				...
				if (aCmd.getCommandLine().hasOption("j")) {
					lTestJobId = CONFIG.getPreferenceInteger(TDConfig.JOB_ID);
				}
				...
				if (lClient.connectToMaster()) {
					lClient.cleanupJob(lTestJobId);
				}
				return null;
			}
			
		}, "cleans-up the server files.");
